...
def lancer_importation(fichier_du_jour):#tout le vrai script
    conn = psycopg2.connect(**db_config)
    cursor = conn.cursor()
    print("Connexion réussie.")

    print("Chargement du fichier XML en mémoire...")
    arbre_xml = ET.parse(fichier_du_jour)
    ...
    cursor.close()
    conn.close()
    print("Importation validé!")

# c'est parti pour le programme
print("debut importation générale")

mois = 5
jour = 1

while mois == 5 or (mois == 6 and jour <= 5):
    
    # Prepa mois
    mois_texte = str(mois)
    if mois < 10:
        mois_texte = "0" + str(mois)
        
    # Prepa jour
    jour_texte = str(jour)
    if jour < 10:
        jour_texte = "0" + str(jour)
        
    # ecriture nom fichier
    fichier_xml = "data/PrixCarburants_quotidien_2026" + mois_texte + jour_texte + ".xml"
    
    print(f"Lancer pour:{fichier_xml}")
    
    # appel fonction
    lancer_importation(fichier_xml)
    
    # our suivant
    jour += 1
    
    # après le 31, c'est le premier juin
    if mois == 5 and jour > 31:
        mois = 6
        jour = 1

print("finish on the light")